...
| 1.5.0     | 7     | 2026-04-19      | **TestFlight** | Phase 2 body-metric goals: `BodyMetricGoal` @Model with direction inference + progress calc. New `SetGoalSheet`, goal card + dashed RuleMark on `MetricDetailView`, progress bar on each `BodyView` row. 12 unit tests + 3 UI smoke tests (one of which caught a second instance of the enum-predicate crash, fixed in SetGoalSheet.save() before upload). Delivery UUID `f80b0bed-6cd8-4773-a2d9-07558b10d98d`. **TestFlight-only** — verify in-app before submitting to review. |

## Next submission will be

- Marketing: **1.6.0** (Phase 3 body-silhouette overlay, or other work).
- Build: **8**.
